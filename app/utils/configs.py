...
class ConfigsParser(object):
    ...
    @staticmethod
    def parse_configs(csection='app'):
        try:
            cur_dir = os.path.dirname(os.path.realpath(__file__))
            '''if local config exists switch to developent and set git ignore dev configs'''
            ConfigsParser.DEVELOPMENT_ENV_CONFIG = os.path.join(
                cur_dir, '../configs/configs_local.ini')
            ConfigsParser.PRODUCTION_ENV_CONFIG = os.path.join(
                cur_dir, '../configs/configs.ini')
